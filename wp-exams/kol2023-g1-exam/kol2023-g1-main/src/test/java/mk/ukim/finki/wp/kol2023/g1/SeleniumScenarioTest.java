...
public class SeleniumScenarioTest {
    ...
    public void test_edit_10pt() {
        ...
        listPage = AddOrEditPlayer.update(this.driver, listPage.getEditButtons().get(itemNum - 1),
                "testName",
                "testBio",
                "100",
                PlayerPosition.G.name(),
                teams.get(0).getId().toString());
        listPage.assertNoError();

        AbstractPage.assertRelativeUrl(this.driver, LIST_URL);
        if (listPage.assertItems(itemNum)) {
            ExamAssert.assertEquals("The updated player name is not as expected.",
                    "testName", listPage.getRows().get(itemNum - 1).findElements(By.tagName("td")).get(0).getText().trim());
        }
